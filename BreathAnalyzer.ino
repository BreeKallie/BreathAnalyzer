...
const int Tolerance = 2; // One Point (Don't Change Plot);
...
void loop()
{
  ...
  if (D12_Inp == 0)
  {
    ...
    if (abs(A0_Out - A0_Last_Out)>=Tolerance || abs(A1_Out - A1_Last_Out)>=Tolerance)
    {
  ...
  calculateAverage(0.0F, RESET);  //RESET
  delay(100);
  if (threshold0 == HIGH)
  {
...
float calculateAverage(float input, bool reset)
{
  static float sum = 0;
  static int count = 0;
  if (reset) {
...
// Write a function to calculate the standard deviation of the last 10 values